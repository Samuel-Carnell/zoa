...
export type RequestValue<TEndpointDefinition extends EndpointDefinition> = Modify<koa.Request, {
    ...
    body: TEndpointDefinition['request']['body'] extends undefined ? undefined : zod.infer<NonNullable<TEndpointDefinition['request']['body']>>;
    endpoint: TEndpointDefinition;
}>;
export type SplitAsUnion<S extends string, D extends string> = string extends S ? never : S extends '' ? never : S extends `${infer T}${D}${infer U}` ? T | SplitAsUnion<U, D> : S;
export type Params<TPath extends string, TPathSegments = SplitAsUnion<TPath, '/'>> = TPathSegments extends `:${infer Param}` ? Param : never;
